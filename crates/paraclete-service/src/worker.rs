...
pub(crate) fn spawn_scan_job_worker(service: ParacleteService) -> tokio::task::JoinHandle<()> {
    ...
    tokio::spawn(async move {
        loop {
            ...
            let now = Utc::now();
            if let Err(e) = service.store().recover_stale_scan_jobs(now, policy).await {
                tracing::error!(error = %e, "recover_stale_scan_jobs");
            }
            ...
            match service.execute_scan_and_persist(req).await {
                ...
                Err(e) => {
                    ...
                    let _ =
                        service.store().complete_scan_job_failure(jid, code, &e.to_string()).await;
                }
            }
        }
    })
